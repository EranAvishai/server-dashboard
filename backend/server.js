...
// ─── AdGuard ──────────────────────────────────────────────────────────────────
// Full /control/stats response includes dns_queries[] + blocked_filtering[]
// — 24-element hourly arrays powering the frontend chart.
...
async function fetchYahooQuote(asset) {
  ...
  // ── Sparkline: last 4 trading day closes for price-direction mini chart ──
  const rawCloses = result.indicators?.quote?.[0]?.close ?? [];
  const sparkline = rawCloses.filter((v) => v != null).slice(-4);

  return {
    key: asset.key, label: asset.label, symbol: asset.symbol,
    price, changePercent, suffix: asset.suffix, asOf,
    sparkline,   // ← new: sent to frontend for the mini direction chart
  };
}
...
async function refreshMarkets() {
  ...
  for (const asset of MARKET_ASSETS) {
    try {
      ...
      results.push({
        key: asset.key, label: asset.label, symbol: asset.symbol,
        price: 0, changePercent: 0, suffix: asset.suffix, asOf: "—", sparkline: [],
      });
    }
  }
  ...
  try {
    ...
    for (const line of lines) {
      ...
      for (const col of cols) {
        const t = col.trim();
        if (/^[0-9]+$/.test(t)) { totalBytesIn += Number(t); break; }
      }
    }
...
// ─── Background refresh — heavy work on timers, endpoints read from cache ─────
